                ...
                <div class="gallery-wrapper">
                    <div class="gallery">
                        <div class="thumbnail">
                            <ul>
                                <li 
                                :class = "{'on': imageNum === index }"
                                v-for = "(little, index) in itemDetail.ali_images"
                                :key = "index"
                                @click = "imageClick(index)"
                                ><img :src = " little + '?x-oss-process=image/resize,w_54/quality,Q_90/format,webp' "></li>
                            </ul>
                        </div>
                        <div class="thumb">
                            <ul>
                                <li class="on"><img :src=" itemDetail.ali_images[imageNum] + '?x-oss-process=image/resize,w_440/quality,Q_90/format,webp' "></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="banner">
                    <div class="sku-custom-title">
                        <div class="params-price">
                            <span><em>¥</em><i>{{itemDetail.price}}</i></span>
                        </div>
                        <div class="params-info">
                            <h4>{{itemDetail.title}}</h4>
                            <h6>{{itemDetail.sub_title}}</h6>
                        </div>
                    </div>
                    <div class="sku-dynamic-params-panel">
                        <div class="sku-dynamic-params clear">
                            ...
                            <ul class="params-colors">
                                <li
                                :class="{'cur':colour.id === $store.state.detail}"
                                v-for="colour in itemDetail.sku_list"
                                :key="colour.id"
                                @click="changeColor(colour.id)"
                                >
                                    <a><i><img :src="'http://img01.smartisanos.cn/' + colour.image + '20X20.jpg'"></i></a>
                                </li>
                            </ul>
                        </div>
                        <div class="sku-dynamic-params clear">
                            ...
                            <div class="params-detail clear">
                                <div class="item-num js-select-quantity">
                                    <span 
                                        class="down"
                                        :class="{'down-disabled':itemNum===1}"
                                        @click = "minusFn"
                                        >-</span>
                                    <span class="num">{{itemNum}}</span>
                                    <span 
                                        class="up"
                                        :class="{'up-disabled':itemNum === itemDetail.limit_num}"
                                        @click = "plusFn"
                                        >+</span>
                                </div>
                            </div>
...
import itemList from "@/assets/dataJS/newItemsData.js"
...
export default {
    ...
    computed:{
        itemDetail(){
            return this.itemList.find(a=>a.sku_id === this.$store.state.detail)
        }
    },
    methods: {
        imageClick(index){      // 点击哪个，图片显示哪个
            this.imageNum = index;
        },
        changeColor(id){     // 改变颜色
            this.$store.state.detail = id;
        ...
        plusFn(){ //产品数量减少
            if(this.itemNum < this.itemDetail.limit_num){
                this.itemNum ++;
            }
        },
        addProductsFn(){ // 向购物车内增加产品
            var obj = Object.assign({},{"num":this.itemNum},this.itemDetail);
            this.$store.commit('addDefinedCard',obj);
        },
        buyFn(){
